...
- **Meerdere flessen tegelijk** (Meer → Meerdere flessen tegelijk, `viewImport`/`aiImport`, kind `import`): lijst plakken of tot 4 foto's van een lijst of staande flessen. Eén aanroep levert een reeks wijnen, de gebruiker vinkt aan wat mee mag; drinkvensters komen van `estimateWindow`, niet van de AI. Dubbelen worden met `matchWine` gemarkeerd en staan standaard uit. Foto's van liggende flessen in een rek hebben geen zin (alleen de bodem in beeld), dat staat ook zo in de tekst.
