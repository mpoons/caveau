#!/bin/sh
# Syntaxcontrole van caveau.html, zonder browser. Rapporteert alleen; repareert niets.
...
# 5. de tests in tests/*.test.js (node --test): de pure regels, geladen uit caveau.html zelf
...
n=$(awk '/<script>/{n++} END{print n+0}' caveau.html)
awk -v dir="$tmp" '
...
if perl -CSD -ne 'print "$.:$_" if /<[^>]*[\x{201C}\x{201D}][^>]*>/' caveau.html >"$tmp/q" && [ -s "$tmp/q" ]; then
...
grep -o "caveau-v[0-9]*" sw.js | head -1 | sed 's/^/service worker: /'
